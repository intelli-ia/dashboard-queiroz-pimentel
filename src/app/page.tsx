...
import NfeItemsPage from '@/components/NfeItemsPage'
...
export default function Home() {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'payables' | 'movements' | 'receipts' | 'nfe_items'>('dashboard')
        ...
        <aside className={`${isSidebarCollapsed ? 'w-20' : 'w-20 md:w-64'} border-r border-border-app bg-card-app/50 backdrop-blur-xl flex flex-col p-4 fixed h-full z-50 transition-all duration-300`}>
          ...
          <nav className="flex-1 space-y-2">
            ...
            >
              ...
              {!isSidebarCollapsed && <span className="font-medium hidden md:block">Movimentos</span>}
            </button>
            ...
            <button
              onClick={() => setActiveTab('nfe_items')}
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all ${activeTab === 'nfe_items'
                ? 'bg-primary-app text-white shadow-lg shadow-primary-app/20'
                : 'hover:bg-white/5 text-muted-foreground hover:text-white'
                }`}
              title="Itens de NFE"
            >
              ...
              {!isSidebarCollapsed && <span className="font-medium hidden md:block">Itens de NFE</span>}
            </button>
          </nav>
